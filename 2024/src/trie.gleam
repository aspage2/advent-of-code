...
pub opaque type Trie {
	Node(
		is_term: Bool, 
		children: dict.Dict(String, Trie)
	)
}
...
pub fn trie_insert(t: Trie, s: String) -> Trie {
	...
	Ok(#(g, rest)) -> {
		...
		let ch = trie_insert(c, rest)
			|> dict.insert(t.children, g, _)
		Node(..t, children: ch)
	}}
}

pub fn is_empty(t: Trie) -> Bool {
...
fn line(sb: string_tree.StringTree, ind: Int, rest: String) -> string_tree.StringTree {
	"| "
		|> string.repeat(ind)
...
fn trie_string_rec(sb: string_tree.StringTree, t: Trie, indent: Int) -> string_tree.StringTree {
	use <- bool.guard(dict.is_empty(t.children), line(sb, indent, case t.is_term {True->"{T}" False->"{}"}))
	use sb, k, v <- dict.fold(t.children, sb)
	let node_name = k <> case t.is_term {
	True -> "T"
	False -> ""
	}
