...
pub type Region = set.Set(Cell)
pub type Edge = #(Cell, Dir)
pub type Perimiter = List(Edge)
...
fn region_rec(
	g: Grid, letter: String, acc: Region, vs: Region
) -> Region {
	use <- bool.guard(set.is_empty(vs), acc)
	let next = vs
		|> set.to_list
		|> list.flat_map(grid.cardinal_neighbors)
		|> list.filter(fn(x){case grid.at(g, x) {
			Ok(s) -> s == letter
			Error(_) -> False
		}})
		|> set.from_list
