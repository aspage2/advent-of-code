...
fn do(vs: VMState, inst: Inst) -> VMState {
	case inst {
	Adv(op) -> combo_op(vs, op) 
			|> int.bitwise_shift_right(vs.reg_a, _) 
			|> write_to_a(vs) 
			|> step
	Bdv(op) -> combo_op(vs, op)
			|> int.bitwise_shift_right(vs.reg_a, _)
			|> write_to_b(vs)
			|> step
	Cdv(op) -> combo_op(vs, op)
			|> int.bitwise_shift_right(vs.reg_a, _)
			|> write_to_c(vs)
			|> step
	Bxl(l) -> vs.reg_b 
			...
			|> write_to_b(vs)
			|> step
	Bxc     -> int.bitwise_exclusive_or(vs.reg_b, vs.reg_c)
			|> write_to_b(vs)
			|> step
	Bst(op) -> combo_op(vs, op)
			|> int.bitwise_and(0b111)
			|> write_to_b(vs)
			|> step
	Jnz(ad) -> case vs.reg_a == 0 {
			   False -> VMState(..vs, pc: ad)
			   True -> vs |> step
			}
	Out(op) -> combo_op(vs, op)
			|> int.bitwise_and(0b111)
...
fn find(prgm: Program, rev_p: List(Int), a: Int) -> Int {
	case rev_p {
		[] -> a
		[h, ..rest] -> {
			let opts = list.range(0, 7)
				|> list.map(fn(x) {
					int.bitwise_shift_left(a, 3) 
						|> int.bitwise_or(x)
				})
			let f = list.filter(opts, fn(x) {
