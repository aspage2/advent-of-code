...
fn parse_grid(txt: String) -> #(Grid, dict.Dict(Int, List(Cell))) {
	...
	let d = {
		...
		let cps = string.to_utf_codepoints(line) 
...
fn line_antinodes(c1: Cell, c2: Cell, g: Grid) -> List(Cell) {
	...
	let l1 = iterator.unfold(c1, fn(c) {
		case !in_grid(c, g) {
			True -> iterator.Done
			False -> iterator.Next(
				c, Cell(c.r - dr, c.c - dc)
			)
		}
	})
	let l2 = iterator.unfold(c2, fn(c) {
		case !in_grid(c, g) {
			True -> iterator.Done
			False -> iterator.Next(
				c, Cell(c.r + dr, c.c + dc) 
...
pub fn day_main(day: util.AdventDay) {
	...
	let assert Ok(anti_nodes) = nodes 
		|> dict.values
