...
import gleam/io
import gleam/pair
import gleam/order import gleam/int
import gleam/result
import gleam/bool
...
fn djikstra(
	...
	let #(new_dist, new_vs, new_up) = next_states(min_node)
		|> list.filter(fn(s) { 
		...
		|> list.fold(#(dist, new_vs, up), fn(acc, st) {
			...
			Ok(other_cost) if other_cost == alt -> #(
				d, vs, dict.upsert(up, st.0, fn(opt) {
					opt 
...
fn search(grid: Grid, start: State, end: Cell) -> #(
	Dict(State, Int), Dict(State, List(State))) {
	djikstra(grid, end, set.from_list([start]), dict.from_list([#(start, 0)]), dict.new()) 
}

fn count_seats_rec(vs: Set(State), visited: Set(State), d: Dict(State, List(State))) -> Int {
...
pub fn day_main(day: util.AdventDay) -> Nil {
	...
	let assert Ok(#(min_k, cost)) = dict.filter(dst, fn(k, _) {k.c == end})
		|> dict.to_list
		|> list.reduce(fn(a, b) {
			case a.1 < b.1 {
			True -> a
