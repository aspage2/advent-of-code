...
fn get_info(g: Grid) -> #(Info, Cell) {
  ...
  use #(inf, st), i <- list.fold(cells, #(
    Info(set.new(), set.new()),
    Cell(-1, -1),
  ))
  case grid.at(g, i) {
    Error(_) -> panic
...
fn get_info2(g: Grid) -> #(Info, Cell) {
  ...
  case grid.at(g, i) {
    Error(_) -> panic
    Ok("#") -> #(
      Info(..inf, walls: set.insert(inf.walls, i1) |> set.insert(i2)),
      st,
    )
...
fn move_box(inf: Info, loc: Cell, d: Dir) -> Result(Info, Nil) {
  ...
  |> result.map(fn(new_inf) { mb(new_inf, loc, new_loc) })
}
...
fn get_boxes_rec(
  ...
  use <- bool.lazy_guard(to_visit == [], fn() { vs |> set.to_list })
    ...
    N | S ->
      ...
      |> list.flat_map(fn(c) {
        ...
        let a =
          ...
          |> result.unwrap([])
        let b =
          get_box_2(inf, grid.move(new_c, E))
          |> result.map(list.wrap)
          |> result.unwrap([])
        list.append(a, b)
      })
  ...
  get_boxes_rec(
    ...
    st,
    dir,
    new_to_visit |> set.to_list,
    vs |> set.union(new_to_visit),
  )
}
...
fn to_string(g: Grid, i: Info, pos: Cell) {
  {
    ...
    list.fold(list.range(0, g.width - 1), sb, fn(sb, c) {
      let c = Cell(r, c)
...
fn gps(inf: Info) -> Int {
  inf.boxes
  |> set.to_list
  |> list.map(fn(cell) { 100 * cell.r + cell.c })
  |> int.sum()
}
