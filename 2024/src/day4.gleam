...
pub fn p1_matches(g: Grid, r: Int, c: Int) -> Int {
	...
	let horiz = c <= g.width - 4 && {
		let x = <<grid_at(g, r, c), grid_at(g, r, c+1), grid_at(g, r, c+2), grid_at(g, r, c+3)>>
		x == <<"XMAS">> || x == <<"SAMX">>
	}
	let vert = r <= g.height - 4 && {
		let x = <<grid_at(g, r, c), grid_at(g, r+1, c), grid_at(g, r+2, c), grid_at(g, r+3, c)>>
		x == <<"XMAS">> || x == <<"SAMX">>
	}
	let diag = c <= g.width - 4 && r <= g.height - 4 && {
		let x = <<grid_at(g, r, c), grid_at(g, r+1, c+1), grid_at(g, r+2, c+2), grid_at(g, r+3, c+3)>>
		x == <<"XMAS">> || x == <<"SAMX">>
	}
	let updiag = c <= g.width - 4 && r >= 3 && {
		let x = <<grid_at(g, r, c), grid_at(g, r-1, c+1), grid_at(g, r-2, c+2), grid_at(g, r-3, c+3)>>
		x == <<"XMAS">> || x == <<"SAMX">>
	}
...
fn p2_matches(g: Grid, r: Int, c: Int) -> Bool {
	...
	let x = <<grid_at(g, r, c), grid_at(g, r+1, c+1), grid_at(g, r+2, c+2)>> 
	let y = <<grid_at(g, r+2, c), grid_at(g, r+1, c+1), grid_at(g, r, c+2)>>

	{x == <<"SAM">> || x == <<"MAS">>} && {y == <<"SAM">> || y == <<"MAS">>}
}

pub fn day_main(day: util.AdventDay) {
	let assert Ok(grid) = util.read_file(day.file) 
